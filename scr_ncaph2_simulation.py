...
with open(r'/vf/users/HiTIF/data/keikhosravia2/ripley_paper/scr_ncaph2_mcmc_results_gpu_3000/scr_ncaph2_bayesian_cell_line_distance_results_final.pkl', 'rb') as f:
    loaded_results = pickle.load(f)

r0 = loaded_results[sys.argv[1]]['r0']
sigma = loaded_results[sys.argv[1]]['sigma']

eight_cell_lines = eight_cell_lines.loc[eight_cell_lines['gene_symbol']==sys.argv[1]]

eight_cell_lines = eight_cell_lines.loc[eight_cell_lines['spots_number'] > 5]
all_ripleys_dict = {}
mean_ripleys_dict = {}
for cell in eight_cell_lines.gene_symbol.unique():
    ripleys_list = []
    gene_symbol_specific_df = eight_cell_lines.loc[eight_cell_lines['gene_symbol'] == cell]
    for ind, row in gene_symbol_specific_df.iterrows():
        ripley_sig = analysis.ripley_k_score(row['spot_coordinates'][0][0], row.area)[1]
        ripley_no_correction_cdf = ripley_sig / np.max(ripley_sig)
...
file_name = str(sys.argv[1]) +'_ripleys_dict.pkl'
full_name = os.path.join('/vf/users/HiTIF/data/keikhosravia2/ripley_paper/scr_ncaph2_results/' + file_name)
with open(full_name, 'wb') as f:
    pickle.dump(all_ripleys_dict, f)
...
for i, (gene_symbol, empirical_k_values) in enumerate(mean_ripleys_dict.items()):
    ...
    fitting_dict[gene_symbol]['sigma_cdf'] = sigma_fitted
    fitting_dict[gene_symbol]['constant_cdf'] = lambda_fitted
    # Convert to microns
    lambda_fitted_micron = lambda_fitted 
    sigma_fitted_micron = sigma_fitted

# Save the results in a pickle file
file_name = str(sys.argv[1]) +'_fitting_dict.pkl'
full_name = os.path.join('/vf/users/HiTIF/data/keikhosravia2/ripley_paper/scr_ncaph2_results/' + file_name)
with open(full_name, 'wb') as f:
    pickle.dump(fitting_dict, f)
...
def process_row(row):
    ...
    mask_path = row['masks_path'].replace('/data1/image_patches/mask/', '/vf/users/HiTIF/data/keikhosravia2/ripley_paper/scr_ncaph2_masks/')
    ...
    # # ripley_based_gaussian_synth_spot_generator
    # seed = np.random.randint(0, 10000, size=1)
    # cov_matrix = np.eye(2) * (fitting_dict[row.gene_symbol]['sigma_cdf']**2)
...
def update_dataframe(result):
    ...
    # for method in ['radial_gaussian_spot_generator', 'cell_based_gaussian_synth_spot_generator', 'ripley_based_gaussian_synth_spot_generator', 'uniform_spot_generator', 'poisson_disk_spot_generator', 'soft_core_spot_generator', 'real_data_spots']:
...
if __name__ == '__main__':
    ...
    full_name = os.path.join('/vf/users/HiTIF/data/keikhosravia2/ripley_paper/eight_cell_lines/scr_ncaph2_latest_results/' + file_name)
