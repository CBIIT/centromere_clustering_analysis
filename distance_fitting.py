...
n_iterations = 1000
T_burnin = 100
num_samples = 500  # Number of Monte Carlo samples
...
def run_mcmc_for_cell_line(cell_type_data, initial_r0, initial_sigma, eta_r0, eta_sigma, n_iterations, T_burnin, num_samples=500):
    ...
    for t in tqdm(range(n_iterations), desc="Iterations", leave=False):
    ...
    with ProcessPoolExecutor(max_workers=180) as executor:
        ...
        for future in tqdm(as_completed(future_to_cell_type), total=len(future_to_cell_type), desc="Cell Lines"):
            ...
            try:
                ...
                with open(f'/vf/users/HiTIF/data/keikhosravia2/ripley_paper/mcmc_results/bayesian_cell_line_distance_results_{cell_type}.pkl', 'wb') as f:
...
cell_line_results = run_mcmc_for_all_cell_lines(eight_cell_lines, num_samples=500)

# Save final results in a pickle file
with open('/vf/users/HiTIF/data/keikhosravia2/ripley_paper/mcmc_results/bayesian_cell_line_distance_results_final.pkl', 'wb') as f:
